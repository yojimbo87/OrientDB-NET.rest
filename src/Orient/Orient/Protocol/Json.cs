...
{
    internal class Json : Dictionary<string, object>
    {
        public Json() { }
            ...
            foreach (var item in jsonObject)
            {
                ...
                if (value.Length >= 2)
                {
                    ...
                    if ((value[0] =='[') && (value[value.Length - 1] == ']'))
                    {
                        ...
                        // OrientDB include whitespaces so check for array of embedded objects
                        // needs to be like: between [ and { cannot be "
                        if ((index != -1 && (value.Substring(0, index).IndexOf('\"') == -1)))
